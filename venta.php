...
function initial()
{
    ...
    $_PAGE['links'] .=
        '<link href="css/plugins/bootstrap-checkbox/bootstrap-checkbox.css" rel="stylesheet">';
    $_PAGE['links'] .= '<link href="css/style.css" rel="stylesheet">';
    $_PAGE['links'] .=
        '<link rel="stylesheet" type="text/css" href="css/plugins/perfect-scrollbar/perfect-scrollbar.css">';
    $_PAGE['links'] .=
        '<link rel="stylesheet" type="text/css" href="css/util.css">';
    $_PAGE['links'] .=
        '<link rel="stylesheet" type="text/css" href="css/main.css">';
    include_once 'header.php'; //include_once "main_menu.php";
    date_default_timezone_set('America/El_Salvador');
    $fecha_actual = date('Y-m-d');
    $id_sucursal = $_SESSION['id_sucursal']; //permiso del script
    $id_user = $_SESSION['id_usuario'];
    $sql_apertura = _query(
        "SELECT * FROM apertura_caja WHERE vigente = 1 AND id_sucursal = '$id_sucursal' AND fecha='$fecha_actual' AND id_empleado = '$id_user'"
    );
    ...
    $id_usuario = $id_user;
    $fecha_actual = date('Y-m-d'); //clientes //factura
    //array clientes
    if (isset($_REQUEST['id_pedido'])) {
        $id_pedido = $_REQUEST['id_pedido'];
        $sql_pedido = _query(
            "SELECT co.*, cl.nombre FROM pedido as co, cliente as cl WHERE co.id_cliente = cl.id_cliente AND co.id_pedido = '$id_pedido'"
        );
        ...
        if ($cuenta != 0) {
            ...
            $fecha_actual = $row_coti['fecha']; // $tipo_doc = $row_coti["tipo_doc"];
            $pedido = $id_pedido;
        }
    } else {
        $nombre = '';
                ...
                <div class="ibox ">
                    ...
                    <div class="ibox-content">
                        ...
                        <div class="row focuss"><br>
                            <!-- BUSCADOR DE PRODUCTOS (CODIGO O DESCRIPCION) -->
                            ...
                            <div class="col-md-7"><br>
                                ...
                                <?php if (isset($_REQUEST['id_pedido'])) {
                                ...
                                ?>
                                <!-- BOTON DE AGREGADO DE SERVICIO 
                                (COMENTADO POR AHORA PORQUE YA EXISTE
                                UN ADMIN PARA ADMINISTRAR LOS SERVICIOS) -->
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-2">
                                <div class="form-group has-info">
                                    ...
                                    <select class="form-control select usage" name="id_empleado" id="id_empleado">
                                        ...
                                        <?php
                                        /**
                                         * OBTENIENDO EMPLEADOS REGISTRADOS EN EL SISTEMA
                                         */ 
                                        $sql_empleado = _query(
                            ...
                            <!-- SELECCION DE REFERENCIAS ALMACENADAS -->
                            ...
                            <div id='form_datos_cliente' class="form-group col-md-3">
                                <div class="form-group has-info">
                                    ...
                                    <select class="form-control select usage" name="id_cliente" id="id_cliente">
                                        <?php
                                        $sqlcli = _query(
                                            "SELECT * FROM cliente WHERE id_sucursal='$id_sucursal' ORDER BY nombre"
                                        );
                                        while (
                                            $row_cli = _fetch_array($sqlcli)
                                        ) {
                                            /**
                                            ...
                                            echo "<option data-retencion='$retiene_10' value='" .
                                                $row_cli['id_cliente'] .
                                                "'";
                                            if ($id_cliente_bd != '') {
                                                if (
                            ...
                            <section>
                                ...
                                <div class="">
                                    <div class="row">
                                                        ...
                                                        <table>
                                                            <tbody id="inventable">
                                                                <?php if (
                                                                    ...
                                                                    // echo $id_pedido;
                                                                    $sql_detalle = _query(
                                                                        "SELECT * FROM pedido_detalle WHERE id_pedido = '$id_pedido'"
                                                                    );
                                                                    $cuenta_pe = _num_rows(
                                                                        $sql_detalle
                                                                        ...
                                                                        while (
                                                                            ...
                                                                            if (
                                                                                ...
                                                                                if (
                                                                                    ...
                                                                                    $sql_exis = _query(
                                                                                        "SELECT stock FROM stock WHERE id_producto = '$id_productov'"
                                                                                    );
                                                                                    $datos_exis = _fetch_array(
                                                                                        $sql_exis
                                                                                    );
                                                                                    $stockv =
                                                                                        $datos_exis[
                                                                                    ...
                                                                                    if (
                                                                                        ...
                                                                                        $sql_res_pre = _fetch_array(
                                                                                            _query(
                                                                                                "SELECT SUM(factura_detalle.cantidad) as reserva FROM factura JOIN factura_detalle ON factura_detalle.id_factura=factura.id_factura WHERE factura_detalle.id_prod_serv=$id_producto AND factura.id_sucursal=$id_sucursal AND factura.finalizada=0 "
                                                                                            )
                                                                                        );
                                                                                        $reserva =
                                                                                            $sql_res_pre[
                                                                                        ...
                                                                                        $sql_res_esto = _fetch_array(
                                                                                            _query(
                                                                                                "SELECT SUM(factura_detalle.cantidad) as reservado FROM factura JOIN factura_detalle ON factura_detalle.id_factura=factura.id_factura WHERE factura_detalle.id_prod_serv=$id_producto AND factura.id_factura=$id_factura"
                                                                                            )
                                                                                        );
                                                                                        $reservado =
                                                                                            $sql_res_esto[
                                                                                        ...
                                                                                        $descripcionp = 0;
                                                                                        $select_rank =
                                                                                            "<select class='sel_r form-control'>";
                                                                                        $anda =
                                                                                            '';
                                                                                        if (
                                                                                            $id_presentacion >
                                                                                        ...
                                                                                        $select .=
                                                                                            '</select>'; // $xdatos['perecedero']=$perecedero; // $xdatos['descripcion']= $descripcion; // $xdatos['id_producto']= $id_productov; // $xdatos['select_rank']= $select_rank; // $xdatos['preciop']= $preciop;
                                                                                        // $xdatos['select']= $select;
                                                                                        // $xdatos['stock']= $stockv;
                                                                                        $sql_e = _fetch_array(
                                                                                            _query(
                                                                                                "SELECT exento FROM producto WHERE id_producto=$id_producto"
                                                                                            )
                                                                                        );
                                                                                        $exento =
                                                                                            $sql_e[
                                                                                        ...
                                                                                        if (
                                                                                            ...
                                                                                            $sqkl = _fetch_array(
                                                                                                _query(
                                                                                                    "SELECT iva FROM sucursal WHERE id_sucursal=$id_sucursal"
                                                                                                )
                                                                                            );
                                                                                            $iva =
                                                                                                $sqkl[
                                                                                        ...
                                                                                        $tr_add .=
                                                                                            "<tr  class='row100 head' id=''>";
                                                                                        $tr_add .=
                                                                                            "<td hidden class='cell100 column10 text-success id_pps'><input type='hidden' id='unidades' name='unidades' value='" .
                                                                                            $unidad .
                                                                                            "'>" .
                                                                                            $id_producto .
                                                                                            '</td>';
                                                                                        $tr_add .=
                                                                                            "<td class='cell100 column30 text-success'>" .
                                                                                            $descripcion .
                                                                                            ' ' .
                                                                                            $exento .
                                                                                            '</td>';
                                                                                        $tr_add .=
                                                                                            "<td class='cell100 column10 text-success' id='cant_stock'>" .
                                                                                            $stockv .
                                                                                            '</td>';
                                                                                        $tr_add .=
                                                                                            "<td class='cell100 column10 text-success'><div class='col-xs-2'><input type='text'  class='txt_box decimal2 " .
                                                                                            $categoria .
                                                                                            " cant' id='cant' name='cant' value='' style='width:60px;'></div></td>";
                                                                                        $tr_add .=
                                                                                            "<td class='cell100 column10 text-success preccs'>" .
                                                                                            $select .
                                                                                            '</td>';
                                                                                        $tr_add .=
                                                                                            "<td hidden class='cell100 column10 text-success descp'><input type'text' id='dsd' class='form-control' value='" .
                                                                                            $descripcionp .
                                                                                            "' class='txt_box' readonly></td>";
                                                                                        $tr_add .=
                                                                                            "<td class='cell100 column10 text-success rank_s'>" .
                                                                                            $select_rank .
                                                                                            '</td>';
                                                                                        $tr_add .=
                                                                                            "<td class='cell100 column10 text-success'><input type='hidden'  id='precio_venta_inicial' name='precio_venta_inicial' value='" .
                                                                                            $preciop .
                                                                                            "'><input type='hidden'  id='precio_sin_iva' name='precio_sin_iva' value='" .
                                                                                            $preciop_s_iva .
                                                                                            "'><input type='text'  class='form-control decimal' readOnly id='precio_venta' name='precio_venta' value='" .
                                                                                            $preciop .
                                                                                            "'></td>";
                                                                                        $tr_add .=
                                                                                            "<td class='ccell100 column10'><input type='hidden'  id='subtotal_fin' name='subtotal_fin' value='0.00'><input type='text'  class='decimal txt_box form-control' id='subtotal_mostrar' name='subtotal_mostrar'  value='0.00' readOnly></td>";
                                                                                        $tr_add .=
                                                                                            '<td class="cell100 column10 Delete text-center"><input id="delprod" type="button" class="btn btn-danger fa"  value="&#xf1f8;"> <a data-toggle="modal" href="ver_imagen.php?id_producto=' .
                                                                                            $id_producto .
                                                                                            '"  data-target="#viewProd" data-refresh="true" class="btn btn-primary btnViw fa"><i class="fa fa-eye"></i></a></td>';
                                                                                        $tr_add .=
                                                                                            '</tr>';
                                                                                    }
                                                                                }
                                        ...
                                        <div class="col-md-3">
                                            <!-- Aqui se capturan los datos para cuando se elige un tipo de pago
                                                (TARJETA, BITCOIN O TRANSFERENCIA)-->
                                            <div hidden class="wrap-table1001 datos_pago">
                                                <div class="table100 ver1 m-b-10">
                                                    <div class="table100-head">
                                                        <table id="inventable2">
                                                            <thead>
                                                                <tr class="row100 head">
                                                                    <th class="success cell100 column100 text-center">DATOS DE PAGO</th>
                                                                </tr>
                                                            </thead>
                                                        </table>
                                                    </div>
                                                    <div class="table101-body">
                                                        <table>
                                                            <tbody>
                                                                <tr class='cdui'>
                                                                    <td class='cell100 column70 text-success'>DUI:</td>
                                                                    <td class='cell100 column30'><input type="text" class="txt_box2 pago_dui" value=""></td>
                                                                </tr>
                                                                <tr class='cope'>
                                                                    <td class='cell100 column70 text-success'>NUMERO DE OPERACION:</td>
                                                                    <td class='cell100 column30'><input type="text" class="txt_box2 pago_operacion" value=""></td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                                                    </div>
...
function cargar_data()
{
  ...
  FROM factura WHERE numero_ref = $n_ref AND finalizada != 1"; //echo $sql_fact;
    ...
    if ($count_fact > 0) {
        ...
        $r_precios = _fetch_array(
            _query("SELECT precios FROM usuario WHERE id_usuario=$id_usuario")
        );
    ...
    ORDER BY cliente.nombre"; //echo $sql_cliente1;
        $id_cliente = 0;
        $nombre_cliente = '';
        $retencion1 = 0;
        $retencion10 = 0;
    ...
    ";
        ...
        $count_fact_det = _num_rows($result_fact_det); //echo $sql_fact_det;
        $total = 0;
        $lista = '';
        for ($i = 0; $i < $count_fact_det; $i++) {
            ...
            $select .= '</select>';
            $sql_cc = _query(
                "SELECT * FROM presentacion_producto WHERE id_pp = '$id_pre'"
            );
            ...
            $sql_s = _fetch_array(
                _query(
                    "SELECT p.id_sucursal,SUM(su.cantidad) as stock FROM producto AS p JOIN stock_ubicacion as su ON su.id_producto=p.id_producto JOIN ubicacion as u ON u.id_ubicacion=su.id_ubicacion  WHERE  p.id_producto ='$id_producto' AND u.bodega=0 AND su.id_sucursal=$id_sucursal"
                )
            );

            $stock_r = $sql_s['stock']; //MODELADO PARA QUE SE MUESTREN PREVENTAS SIN LIMITE DE TIEMPO //$hoy=date("Y-m-d");
            $sql_res_pre = _fetch_array(
                _query(
                    "SELECT SUM(factura_detalle.cantidad) as reserva FROM factura JOIN factura_detalle ON factura_detalle.id_factura=factura.id_factura WHERE factura_detalle.id_prod_serv=$id_producto AND factura.id_sucursal=$id_sucursal AND factura.finalizada=0 "
                )
            ...
            $descprod = $descripcion; //$ubica=ubicacionn($id_posicion);
            $ubicacion = '';
            ...
            $lista .=
                "<td class='cell100 column30 text-success'>" .
                $descripcion .
                "<input type='hidden' id='exento' name='exento' value='" .
                $exento .
                "'>" .
                '</td>';
            $lista .=
                ...
                '</td>';
            $lista .=
                "<td class='cell100 column10 text-success'><input type='text'  class='form-control  $categoria cant' id='cant' name='cant' value=" .
                $cc .
                " style='width:60px;'></td>";
            $lista .=
                "<td class='cell100 column10 text-success preccs'>" .
                ...
                '</td>';
            $lista .=
                "<td hidden class='cell100 column10 text-success descp'>" .
                "<input type'text' id='dsd' value='" .
                $descripcionp .
            ...
            $lista .=
                ...
                $precio_venta .
                "' ><input type='hidden'  id='precio_sin_iva' name='precio_sin_iva' value='" .
                round($precio_venta / $iva, 8, PHP_ROUND_HALF_DOWN) .
                "'><input type='text'  class='form-control decimal' id='precio_venta' name='precio_venta' value='" .
                $precio_venta .
                "' ></td>";
            $lista .=
                "<td class='ccell100 column10'>" .
                "<input type='hidden'  id='subtotal_fin' name='subtotal_fin' value='" .
                $subtotal .
                "'>" .
                "<input type='text'  class='decimal form-control' id='subtotal_mostrar' name='subtotal_mostrar'  value='" .
                round($subtotal, 2) .
                "'readOnly>" .
                '</td>';
            $lista .=
                "<td class='cell100 column10 Delete text-center'><input id='delprod' type='button' class='btn btn-danger fa'  value='&#xf1f8;'>" .
                '<a data-toggle="modal" href="ver_imagen.php?id_producto=' .
                $id_producto .
                '"  data-target="#viewProd" data-refresh="true" class="btn btn-primary btnViw fa"><i class="fa fa-eye"></i></a>' .
                '</td>';
            $lista .= '</tr>';
        } //servicios
        $sql_servicios = _query(
            "SELECT * FROM  factura_detalle where id_factura=$id_factura AND id_prod_serv=-9999 AND id_presentacion=-9999"
        );
        while ($row_serv = _fetch_array($sql_servicios)) {
            ...
            $lista .=
                "<td class='cell100 column10 text-success id_pps' hidden=''>";
            $lista .=
                "<input id='unidades' name='unidades' value='1' type='hidden'>" .
                +$id_servicio .
                '';
            $lista .= '</td>';
            $lista .=
                "<td class='cell100 column30 text-success descripcion_insumo' >" .
                $nombre_servicio .
                '';
            $lista .=
            ...
            $lista .=
                "<div><input disabled class=form-control decimal2 cant' id='cant' name='cant' value='1' style='width:60px;' type='text'>";
            $lista .= '</div>';
            $lista .= '</td>';
            ...
            $lista .= '</td>';
            $lista .=
                "<td style='display:none' class='cell100 column10 text-success descp'>";
            $lista .=
                "<input type'text'='' id='dsd' class='form-control' value='SERVICIO' readonly=''>";
            $lista .= '</td>';
            ...
            $lista .= "<td class='cell100 column10 text-success'>";
            $lista .=
                "<input id='precio_venta_inicial' name='precio_venta_inicial' value='" .
                $monto .
                "' type='hidden'>";
            $lista .=
                "<input id='precio_sin_iva' name='precio_sin_iva' value='" .
                round($monto / 1.13, 4) .
                "' type='hidden'>";
            $lista .=
                "<input class='form-control decimal' readonly='' id='precio_venta' name='precio_venta' value='" .
                $monto .
                "' type='text'>";
            $lista .= '</td>';
            $lista .= "<td class='ccell100 column10'>";
            $lista .=
                "<input id='subtotal_fin' name='subtotal_fin' value='" .
                $subtotal .
                "' type='hidden'>";
            $lista .=
                "<input class='decimal txt_box form-control' id='subtotal_mostrar' name='subtotal_mostrar' value='" .
                $subtotal .
                "' readonly='' type='text'>";
            $lista .= '</td>';
            $lista .= "<td class='cell100 column10  text-center'>";
            $lista .=
                "<input id='delprod' class='btn btn-danger fa fa-trash Delete' value='' type='button'>";
            $lista .= '</td>';
            $lista .= '</tr>';
        }
        $select_vendedor = '';
        $sqlemp = _query(
            "SELECT id_empleado, nombre FROM empleado WHERE id_sucursal='$id_sucursal' AND id_tipo_empleado=2"
        );
        ...
        if ('COF' == $alias_tipodoc) {
            # code...
            $select_tipo_impresion .=
                "<option value='COF' selected>FACTURA CONSUMIDOR FINAL</option>";
        } else {
            # code...
            $select_tipo_impresion .=
...
function consultar_stock()
{
    ...
    if ($tipo == 'D') {
        ...
        $sql_aux = _query(
            "SELECT id_producto FROM producto WHERE barcode='$id_producto'"
        );
        if (_num_rows($sql_aux) > 0) {
            ...
            $sql_aux = _query(
                "SELECT id_pp as id_presentacion, id_producto FROM presentacion_producto WHERE id_pp='$id_producto' AND activo='1'  OR  barcode='$id_producto' AND activo='1' "
            );
            if (_num_rows($sql_aux) > 0) {
                $dats_aux = _fetch_array($sql_aux);
                $id_producto = $dats_aux['id_producto'];
    ...
    if ($nrow1 > 0) {
        ...
        ORDER BY presentacion_producto.unidad ASC");
                ...
                while ($row = _fetch_array($sql_p)) {
                    ...
                    $select .=
                        "<option value='" . $row['id_presentacion'] . "'";
            ...
            $xdatos['typeinfo'] = 'Error';
            $xdatos['msg'] =
                'El codigo ingresado no pertenece a nungun producto';
            echo json_encode($xdatos);
...
function insertar()
{
    ...
    /**
     * VARIABLES QUE CONTIENEN INFORMACION DE LA OPERACION
     * CON PAGO REALIZADO YA SEA POR TARJETA, BITCOIN O TRANSFERENCIA
     */
    $pago_dui = $_REQUEST['pago_dui'];
    $pago_operacion = $_REQUEST['pago_operacion'];
    ...
    if ($tipo_impresion == 'TIK') {
        ...
        $data_numdoc = ['correlativo_dispo' => $correlativo_dispo + 1];
    ...
    if ($tipo_impresion == 'TIK') {
        ...
        # code...
        $swl = _fetch_array(
            _query("SELECT * FROM sucursal where id_sucursal=$id_sucursal ")
        );
        $serie = $swl['serie_cof'];
        $sql_ult = _query(
            "SELECT MAX(CONVERT(num_fact_impresa,UNSIGNED INTEGER)) as ultimo FROM factura WHERE id_sucursal=$id_sucursal AND tipo_documento='COF' "
        );
        ...
        $serie = $swl['serie_ccf'];
        $sql_ult = _query(
            "SELECT MAX(CONVERT(num_fact_impresa,UNSIGNED INTEGER)) as ultimo FROM factura WHERE id_sucursal=$id_sucursal AND tipo_documento='CCF' "
        );
        $num_rows_ul = _num_rows($sql_ult);
        if ($num_rows_ul > 0) {
            # code...
    ...
    if ($id_factura == '') {
      ...
      'id_empleado' => $id_vendedor,*/,
            ...
            'precio_aut' => $precio_aut,
            'clave' => $clave,
            'id_server' => '0', //'cargo_tarjeta' => $cargo_tarjeta,
        ];
    ...
    if ($cuantos > 0) {
        ...
        foreach ($array as $fila) {
            ...
            if ($id_producto != '-9999') {
                ...
                $orig = _fetch_array(
                    _query(
                        "SELECT ubicacion.id_ubicacion FROM ubicacion WHERE ubicacion.bodega=0 AND ubicacion.id_sucursal=$id_sucursal"
                    )
                );
                $origen = $orig['id_ubicacion'];
                $sql = _query(
                    "SELECT * FROM stock_ubicacion WHERE stock_ubicacion.id_producto=$id_producto AND stock_ubicacion.id_ubicacion=$origen AND stock_ubicacion.cantidad!=0 ORDER BY id_posicion DESC ,id_estante DESC "
                );
                ...
                if ($a_transferir > 0) {
                    /*verificamos si se desconto algo de stock_ubicacion*/ if (
                        $a_transferir != $cantidad
                    ) {
                        /*si entra aca significa que se descontaron algunas unidades de stock_ubicacion y hay que descontarlas de stock y lote*/ /*se insertara la diferencia entre el stock_ubicacion y la cantidad a descontar en la tabla de movimientos pendientes*/ $table1 =
                            'movimiento_producto_detalle';
                        $cant_total =
                            $existencias - ($cantidad - $a_transferir);
                        $form_data1 = [
              ...
              ORDER BY vencimiento");
                        ...
                        /*significa que no hay nada en stock_ubicacion y no se puede descontar de stock_ubicacion ni de stock*/ /*se insertara todo en la tabla de movimientos pendientes*/ $table1 =
                            'movimiento_producto_pendiente';
                        $cant_total = $existencias - $cantidad;
                        $form_data1 = [
                            'id_movimiento' => $id_movimiento,
                            'id_producto' => $id_producto,
                ...
                } /*Hay suficientes unidades en  el stock_ubicacion para realizar el descargo y se procede normalmente*/ else {
                        ...
                        while ($row = _fetch_array($sql)) {
                            ...
                            if ($cantidad_a_descontar > 0) {
                                ...
                                if ($entrada_lote == 0) {
                                    ...
                                    if (
                                        $entrada_lote - $cantidad_a_descontar >
                                        0
                                    ) {
                                        # code...
                                        $table = 'lote';
        ...
        if ($a && $b && $c && $x && $z && $k && $j && $l && $cre) {
            ...
            $xdatos['typeinfo'] = 'Success';
            $xdatos['msg'] =
                'Registro  Numero: <strong>' .
                $numero_doc .
                '</strong>  Guardado con Exito !';
            ...
            $xdatos['typeinfo'] = 'Error';
            $xdatos['msg'] =
                'Registro no pudo ser ingresado!' .
                $a .
                '-' .
                $b .
                '-' .
...
function cons_rank()
{
  ...
  $limit");
    ...
    if (_num_rows($sql_rank) > 0) {
        ...
        $sqlq = _query(
            "SELECT precio FROM presentacion_producto WHERE id_presentacion='$id_presentacion'"
        );
...
function getpresentacion()
{
    ...
    $sql = _fetch_array(
        _query(
            "SELECT * FROM presentacion_producto WHERE id_pp=$id_presentacion"
        )
    );
    $precio = $sql['precio'];
    $unidad = $sql['unidad'];
    ...
    $sql_e = _fetch_array(
        _query("SELECT exento FROM producto WHERE id_producto=$id_producto")
    );
    $exento = $sql_e['exento'];
    $select_rank = "<select class='sel_r precio_r form-control'>";
    $id_sucursal = $_SESSION['id_sucursal'];
    ...
    $select_rank .= '</select>';
    $des =
        "<input type='text' id='ss' class='txt_box form-control' value='" .
        $descripcion .
        "' readonly>";
    ...
    if ($exento == 1) {
        ...
        # code...
        $sqkl = _fetch_array(
            _query("SELECT iva FROM sucursal WHERE id_sucursal=$id_sucursal")
        );
        $iva = $sqkl['iva'] / 100;
        $iva = 1 + $iva;
        $xdatos['preciop_s_iva'] = round(
...
function insertar_preventa()
{
    ...
    $id_vendedor = $_POST['id_vendedor'];
    $cuantos = $_POST['cuantos'];
    $array_json = $_POST['json_arr']; //  IMPUESTOS
    $total_percepcion = $_POST['total_percepcion'];
    $subtotal = $_POST['subtotal'];
    $sumas = $_POST['sumas'];
    $suma_gravada = $_POST['suma_gravada'];
    ...
    if ($id_factura == 0) {
        ...
        $sql_num = _query(
            "SELECT * FROM factura where id_factura=$id_factura AND finalizada = 0"
        );
    ...
    if ($cuantos > 0) {
        ...
        if ($a && $b && $c) {
            ...
            $xdatos['typeinfo'] = 'Error';
            $xdatos['msg'] =
                'Registro no pudo ser ingresado!' . $a . '-' . $b . '-' . $c;
        }
    }
    echo json_encode($xdatos);
}
...
function getcode()
{
    ...
    $id_usuario = $_SESSION['id_usuario'];
    $sql = _query(
        "SELECT * FROM precio_aut where clave='$clave' and aplicado=0"
    );
    if (_num_rows($sql) > 0) {
